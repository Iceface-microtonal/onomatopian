...
> **権利について / Rights**: 本リポジトリのソースコード・音声サンプル・生成モデル
> (音素バイアス表・立法規則等) は、作品の仕組みの公開と検証のために閲覧可能にしています。
> **再利用・再配布・派生作品への組み込みは許諾していません** (all rights reserved —
> LICENSE ファイルが無いのは意図的な選択です)。引用・研究・取材のご相談は issue へ。
>
> The source code, voice samples, and generation model in this repository are published
> for transparency and verification of how the work operates. **Reuse, redistribution,
> and derivative works are not licensed** (all rights reserved — the absence of a LICENSE
> file is intentional). Please open an issue for citation, research, or press inquiries.

## 内容

- `iceface_onomatoi.html` — 現行版 (詳細は下記「現行版が旧版から変わった点」)
- `about_onomatoi.html` — 現行版の解説ページ (日英対応)
- `ConsonantsOnomatoi/` — 現行版の音声サンプル 127 mp3 (cv_*/v_*/n_N/v_*_nn/diph_*、48kbps mono)
- `iceface_onomatopian.html` — 旧版 (2026-06-22 時点でアーカイブ、以後更新なし)
- `about.html` — 旧版の解説ページ
- `Consonants/` — 旧版の音声サンプル 108 mp3
...
- **描く** — キャンバスに 1 ストローク描くと 6 軸を抽出し、シニフィアンを生成・発声する。
  **かたちの意味論 4 軸** (size/sharp/texture/bright) + **口のかたち 2 軸** (round/open —
  その形を発音するならどんな口になるか。丸→あ/横棒→い/小円→う/斜めに伸びた楕円→え/
  少し縦長の丸→お)。開いた弧 (⊃⊂∩∪ のような形) や開いた一角 (＜＞∧∨ のような形) は
  「母音+ん」「子音+ん」の最小の語になる。
...
## 現行版が旧版 (iceface_onomatopian.html) から変わった点
...
- **口のかたち 2 軸 (round/open)**: 描いた形を「発音するときの口」としても読み、あいうえお
  5 母音を口の形から選ぶように。斜めに伸びた楕円は傾きの向きによらず え になる。
- **開いた弧・開いた一角**: ⊃⊂∩∪ のような開いた弧は「母音+ん」、＜＞∧∨ のような開いた
  一角は「子音+ん」の最小の語になる (向きは問わない)。
- **や/にゃ 等の拗音・半母音**: 実際にうねった (曲がった) 描線でないと選ばれにくくなり、
  まっすぐな線がそれらしくない語になる誤読が減った。
- **隣り合うモーラの言いやすさ**: 同じ場所で似た構えの子音が連続するとき (例: だ→ざ)、
  後ろの母音が入れ替わって言いやすい形に調整される。

データ形式は `FB_VERSION = 4` (旧版の 👍👎 蓄積とは非互換 — legalize でモーラ構造自体が
変わるため)。
...
Voice ・調音設計の検討：しまなかこう (koh Shimanaka)
